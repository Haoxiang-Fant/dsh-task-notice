...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PricingSection(props) {
      ...
      {
        const prov = (snap.config && Array.isArray(snap.config.providers)) ? snap.config.providers : []
        for (const p of prov) {
            ...
            el('div', { className: 'tn-daily-scroll' },
              daily.map((d) => el('div', { className: 'tn-daily-day', key: d.date },
                ...
                d.cost > 0
                  ...
                  : null,
                d.models.map((m, mi) => el('div', { className: 'tn-daily-model', key: m.model },
                  el('span', { className: 'tn-daily-dot', style: { background: ANALYSIS_COLORS[mi % ANALYSIS_COLORS.length] } }),
