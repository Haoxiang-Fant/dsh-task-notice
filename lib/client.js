...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function showNativeNotify(frame, cfg) {
      ...
      const title = t(NATIVE_TYPE_TITLE[frame.type])
    ...
    function PopupItem({ frame, seconds, onClose }) {
      ...
      const title = POPUP_TITLE[frame.type] || 'popupTitle'
      ...
      return el('div', { className: 'tn-popup ' + kind },
        el('div', { className: 'tn-popup-head' },
          el('div', { className: 'tn-popup-title' }, t(title)),
      ...
      return el('div', { className: 'tn-section' },
        ...
        el('div', { className: 'tn-cards' },
          ...
          el('div', { className: 'tn-card' },
            ...
            el('label', { className: 'tn-check' },
              el('input', { type: 'checkbox', checked: snap.config ? snap.config.notifyOnApproval : true, onChange: (e) => { void api.updateConfig({ notifyOnApproval: e.target.checked }).catch((err) => setData({ status: 'error', stats: data.stats, error: String(err && err.message ? err.message : err) })) } }),
              el('span', null, t('notifyApproval') + ' — ' + t('notifyApprovalDesc'))),
            el('label', { className: 'tn-check' },
              el('input', { type: 'checkbox', checked: snap.config ? snap.config.notifyOnQuestion : true, onChange: (e) => { void api.updateConfig({ notifyOnQuestion: e.target.checked }).catch((err) => setData({ status: 'error', stats: data.stats, error: String(err && err.message ? err.message : err) })) } }),
    ...
    async function apply(ctx) {
      ...
      const DEFAULT_CFG = { webNotify: true, webNotifyBackgroundOnly: true, notifyOnTurn: true, notifyOnGoal: true, notifyOnApproval: true, notifyOnQuestion: true }
