  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        addRule: '添加规则',
        deleteRule: '删除',
        rulePriorityNote: '规则自上而下逐条判定,首个命中调用时刻的规则生效并按倍率缩放基础价,无命中按基础价(类似条件格式的优先级)。',
      ...
      en: {
        ...
        addRule: 'Add rule',
        deleteRule: 'Delete',
        rulePriorityNote: 'Rules are evaluated top-down; the first rule matching the call time applies and scales the base price by its multiplier, no match → base price (like conditional formatting priority).',
    ...
    function StatsSection(props) {
      ...
      return el('div', { className: 'tn-section', ref: sectionRef },
        ...
        el('div', { className: 'tn-tabs' },
          el('button', { className: 'tn-tab' + (view === 'today' ? ' active' : ''), onClick: () => setView('today') }, t('todayTab')),
    ...
    function ProviderIdManageSection(props) {
      ...
      const rows = list.map((p) => el('tr', { key: p.id },
        el('td', null, p.id),
        el('td', null, p.displayName || p.id),
        el('td', null, p.keyEnv || '—'),
        el('td', null, Array.isArray(p.models) && p.models.length > 0 ? fmt(p.models.length) + ' · ' + p.models.slice(0, 6).join(', ') + (p.models.length > 6 ? '…' : '') : t('modelsNone')),
        el('td', null,
    ...
    function SettingsSection(props) {
      ...
      return el('div', { className: 'tn-settings-section' },
        ...
        el(TokenPlanKeyManageSection, { hooks: props.hooks, api }),
    ...
    function PricingSection(props) {
      ...
      // 主体峰谷规则 → 可保存形态(enabled + 合法规则;无规则返回 null)
      const cleanSubjectPeak = (pk) => {
        ...
        for (const r of pk.rules) {
          ...
          rules.push({ windows, weekdaysOnly: r.weekdaysOnly !== false, multiplier: Number.isFinite(mult) && mult > 0 ? Math.min(1000, mult) : 2 })
        }
        return rules.length > 0 ? { enabled: pk.enabled === true, rules } : null
      }
      const sanitizeProviders = (provSrc) => {
        ...
        for (const pid of Object.keys(provSrc)) {
          ...
          if (useC && Object.keys(ownPrice).length === 0 && pPeak === null) continue
          const prow = { useCollective: useC, price: ownPrice }
      ...
      const onSave = async () => {
        try {
          ...
          for (const name of Object.keys(pricing.models || {})) {
            ...
            const hasContent = (row.enabled === true && Object.keys(price).length > 0) || mPeak !== null || Object.keys(providers).length > 0
            if (!hasContent) continue
            const outRow = { enabled: row.enabled === true && Object.keys(price).length > 0, price }
      ...
      for (const p of dirProviders) {
        ...
        for (const m of p.models) {
          if (typeof m === 'string' && m !== '') {
            modelSet.add(m)
            if (!providerOfModel.has(m)) providerOfModel.set(m, new Set())
            providerOfModel.get(m).add(p.id || '')
          }
        }
      }
      ...
      // ── 行数据读取 / 修改 ─────────────────────────────────────────────────
      const rowOf = (model) => {
        const dr = (draft && draft.pricing.models && draft.pricing.models[model]) || null
        return dr || (basePricing.models && basePricing.models[model]) || { enabled: false, price: {} }
      }
      // 卡片显示价:已自定义 → 集体价;未自定义 → 官方价(目录收录)或留空(继承全局默认)
      const displayPrice = (model) => {
        const row = (pricing.models && pricing.models[model]) || null
        if (row && row.enabled === true) return row.price || {}
      ...
      const subjectPeakOf = (model, pid) => {
        ...
        return peak && Array.isArray(peak.rules) ? peak : { enabled: false, rules: [] }
      }
      ...
      const providersFor = (model, row) => {
        ...
        for (const p of dirProviders) { if (p && p.id && Array.isArray(p.models) && p.models.includes(model)) set.add(p.id) }
      ...
      const rulesSummary = (peak) => {
        ...
        return peak.rules.map((r) => windowsToText(r.windows) + ' ×' + String(r.multiplier !== undefined && r.multiplier !== '' ? r.multiplier : '?') + (r.weekdaysOnly !== false ? '(' + t('weekdaysShort') + ')' : '')).join(' / ')
      }
      ...
      if (subject !== null) {
        ...
        const title = t('peakSubjectTitle') + ' — ' + subject.model + (subject.provider ? ' · ' + providerName(subject.provider) + '(' + subject.provider + ')' : ' · ' + t('collectiveLabel'))
        return el('div', { className: 'tn-price-page' },
          ...
          el('div', { className: 'tn-card' },
            ...
            peak.rules.map((rule, i) => el('div', { className: 'tn-rule-row', key: i },
              ...
              el('label', { className: 'tn-price-field tn-rule-mult' },
                ...
                el('input', {
                  ...
                  onChange: (e) => patchSubjectPeak(subject.model, subject.provider, (pk) => { pk.rules[i] = { ...rule, multiplier: e.target.value === '' ? '' : Number(e.target.value) } }),
                })),
              el('label', { className: 'tn-check' },
                el('input', {
                  type: 'checkbox',
                  checked: rule.weekdaysOnly !== false,
                  onChange: (e) => patchSubjectPeak(subject.model, subject.provider, (pk) => { pk.rules[i] = { ...rule, weekdaysOnly: e.target.checked } }),
                }),
            ...
            el('div', { className: 'tn-add-row', style: { marginTop: '8px' } },
              el('button', {
                className: 'tn-btn',
                onClick: () => patchSubjectPeak(subject.model, subject.provider, (pk) => { pk.rules.push({ windows: [{ start: 1, end: 4 }, { start: 6, end: 10 }], weekdaysOnly: true, multiplier: 2 }) }),
              }, t('addRule'))),
    ...
    function TodaySection(props) {
      ...
      for (const row of rows) {
        ...
        for (const m of models) {
          ...
          let agg = modelMap.get(name)
          if (agg === undefined) {
            agg = { label: name, model: name, provider: m.provider || '', calls: 0, cacheIn: 0, cacheOut: 0, output: 0, cacheWrite: 0, reasoning: 0, tokens: 0, cost: 0 }
            modelMap.set(name, agg)
          }
          agg.calls += m.calls
          agg.cacheIn += m.cacheIn
            ...
            el('div', { className: 'tn-pie-wrap' },
              el('div', { className: 'tn-pie-box', onMouseLeave: () => setHover(null) },
                ...
                hovered !== null
                  ? el('div', { className: 'tn-pie-tip', style: { left: hovered.x, top: hovered.y } },
                    el('div', { style: { fontWeight: 600 } }, hovered.m.model),
              ...
              el('div', { className: 'tn-legend' },
                pieModels.map((m, i) => el('div', { className: 'tn-legend-item', key: m.model },
                  ...
                  el('span', { className: 'tn-legend-name' }, m.model),
              ...
              el('table', { className: 'tn-table' },
                el('thead', null,
                  el('tr', null,
                    ...
                    el('th', { className: 'num' }, t('money')))),
                el('tbody', null,
                  modelRows.map((m) => el('tr', { key: m.model },
                    el('td', null, m.model),
                    el('td', { className: 'num', title: fmtExact(m.calls) }, fmt(m.calls)),
                    el('td', { className: 'num', title: fmtExact(m.cacheIn) }, fmt(m.cacheIn)),
                    el('td', { className: 'num', title: fmtExact(m.cacheOut) }, fmt(m.cacheOut)),
          ...
          ? el('div', { style: { display: 'flex', flexDirection: 'column', gap: '16px' } },
            ...
            el('div', { className: 'tn-pie-wrap' },
              ...
              el('div', { className: 'tn-legend' },
                pieModels.map((m, i) => el('div', { className: 'tn-legend-item', key: m.model },
                  el('span', { className: 'tn-legend-dot', style: { background: ANALYSIS_COLORS[i % ANALYSIS_COLORS.length] } }),
                  el('span', { className: 'tn-legend-name' }, m.model + (m.brand && m.brand !== '其他' ? ' (' + m.brand + ')' : '')),
    ...
    function TopBannerHost(props) {
      ...
      React.useEffect(() => {
        let alive = true
        api.getPriceCatalog().then((res) => {
          if (alive && res !== null && typeof res === 'object' && res.ok === true && res.value && typeof res.value === 'object') setCatalog(res.value)
        }).catch(() => { /* 目录不可用时价格横幅隐藏 */ })
        return () => { alive = false }
      }, [])
      ...
      /** 模型是否已有规定价格:自定义集体价 / Provider 专属价 / 官方目录任一命中。 */
      const modelHasPrice = (model) => {
        if (typeof model !== 'string' || model === '') return true
        const row = pricing && pricing.models ? pricing.models[model] : null
          ...
          try {
            ...
            for (const entry of res.value.entries) { // 服务端已按时间倒序
              if (entry && typeof entry.model === 'string' && entry.model !== '' && entry.model !== 'unknown' && !modelHasPrice(entry.model)) {
                setProbe({ model: entry.model, at: Number(entry.at) > 0 ? Number(entry.at) : 0 })
                return
              }
            }
            setProbe(null)
      ...
      return el('div', { className: 'tn-banner-host' },
        ...
        showPriceBanner
          ? el('div', {
            ...
            el('span', { className: 'tn-banner-icon' }, '💰'),
            el('span', { className: 'tn-banner-text' }, t('bannerPriceText', { model: priceModel })),
            el('button', {
              className: 'tn-banner-close', 'aria-label': t('close'),
        ...
        editing !== null
          ? el('div', {
            ...
            el('div', { className: 'tn-modal' },
              el('div', { className: 'tn-modal-title' }, t('bannerPriceEditTitle', { model: editing.model })),
    ...
    async function apply(ctx) {
      ...
      const pushFrame = (frame) => {
        ...
        if (frame !== null && typeof frame === 'object' && frame.type === 'turn' && typeof frame.model === 'string' && frame.model !== '') {
          next.lastModel = { model: frame.model, at: (typeof frame.atMs === 'number' && frame.atMs > 0) ? frame.atMs : Date.now() }
        }
        store.set(next)
