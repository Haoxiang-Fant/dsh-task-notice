 ...
 *   assistant/message   → fallback: some hosts attach usage to the message;
 *                         only counted when no usage chunk was seen for the
 *                         same step (prevents double counting)
...
function createSessionState() {
  return {
    ...
    stepNo: null,
    stepUsageSeen: false, // usage-chunk seen for the current step (anti-double-count)
    ...
    onSessionEvent(session, event) {
      ...
      switch (event.type) {
        ...
        case 'assistant/message': {
          // Fallback for hosts that attach usage to the final message; only
          // counted when the step never saw a usage chunk, so a host that
          // reports both cannot double-count.
          const usage = event.data?.usage
          if (!usage || s.stepUsageSeen) return
          const buckets = bucketsFromEventUsage(usage)
          addBuckets(s.cumulative, buckets)
          if (s.turn) {
            const stepNo = s.stepNo ?? 0
        ...
        case 'goal/change': {
          ...
          if (meta.operation === 'create') {
            ...
            if (goal && typeof handlers.onGoal === 'function') {
              try {
                handlers.onGoal({ session, goal, buckets })
              } catch (error) {
