 ...
 *    多少 tokens。0.3.7 修复 Token Plan 的 Provider ID:加权金额改按「各模型按
 *    模型价的消耗金额占比」摊薄订阅费(旧版按 tokens 占比摊薄,同计划所有模型
 *    性价比恒同、失去区分度),ratio = 该模型调用量 ÷ 加权后的使用金额,只统计
 *    当前生效 TID(当前周期),无数据时标注「正在统计」;
 ...
 *  - 0.3.7 修复「模型归属的 Provider ID」(Token Plan 汇总):消耗聚合的 Key
 *    身份严格按 Provider 目录(自动抓取的全局模型设置)判定——目录声明了该
 *    模型的 Provider ID 优先于运行时路由 / 账本 Key 标签,目录未收录保持原身份;
 *  - 0.3.7 模型品牌不再绑定厂商:Provider ID 是用户自己设置的,品牌显示一律读
 *    本地数据 = 目录里归属 Provider 的显示名(不按模型名前缀硬编码厂商)。
 */

import { priceFor, costOf, tierForEntry, isLegacyEntry } from './pricing.js'
import { providerOfModel, displayNameOf } from './providers.js'
...
// ── 品牌归属(0.3.7:读本地数据,不绑定模型名与厂商)─────────────────────────

/**
 * 模型的品牌显示(0.3.7 修复):删除内置「模型名前缀 → 厂商」表——Provider ID
 * 是用户自己设置的,归属一律读本地数据(Provider 目录 = 全局模型设置 + 账本观测):
 * 目录声明了该模型的 Provider(其显示名)→ 运行时路由(其显示名)→「其他」。
 */
function brandLabelOf(providers, provider, model) {
  const owner = providerOfModel(providers, model, provider)
  const pid = owner !== null && owner !== '' ? owner : (typeof provider === 'string' ? provider.trim() : '')
  return pid === '' ? '其他' : displayNameOf(pid, providers)
}
 ...
 *   peak       - 峰谷配置
 *   providers  - Provider 目录(dirState().list;0.3.7 模型归属严格按目录判定)
 ...
 * }
 */
export function computeAnalysis({ entries, pricing, plans, peak, fromMs, toMs, now, providers }) {
  ...
  // 0.3.7 修复(模型归属的 Provider ID):消耗聚合的 Key 身份严格按 Provider 目录
  // 判定——「每个 Provider ID 下方的模型」自动来自全局模型设置。同名模型按实际
  // 调用路由归属(路由自己是目录行且声明了该模型 → 直接命中,不串台);路由缺失
  // 或不声明该模型时按目录顺序取第一个声明者;目录未收录保持原身份(账本观测兜底)。
  // 回本 / 性价比 / 每日加权 / Key 额度 / 饼图全部同源。
  const providerList = Array.isArray(providers) ? providers : []
  const ownedEntries = providerList.length === 0
    ? entries
    : entries.map((e) => {
      if (e === null || typeof e !== 'object') return e
      const owner = providerOfModel(providerList, e.model, typeof e.provider === 'string' ? e.provider : '')
      return owner === null || owner === e.key ? e : { ...e, key: owner }
    })
  ...
  const { planModelMap, cycleStats } = collectCycleStats(ownedEntries, planCtx, today)
  ...
  const models = [...modelMap.values()]
    .map((m) => ({ ...m, brand: brandLabelOf(providerList, m.provider, m.model) }))
  ...
  // 0.3.7 修复 Token Plan 的 Provider ID 性价比计算:加权金额改按「各模型按
  // 模型价的消耗金额占比」摊薄订阅费(旧版按 tokens 占比摊薄,数学上恒等于
  // 周期 tokens ÷ 订阅费,同计划所有模型性价比相同、失去区分度);修复后
  // ratio = 该模型调用量 ÷ 加权后的使用金额 = 1 块钱可兑换的 tokens 数量,
  // 同计划下按模型价效率区分排名。只统计**当前生效 TID**(当前周期)的数据
  // ——旧 TID(上一轮 / 冻结历史)不再参与排名,其数据经「历史」按钮查阅;
  // 计划尚无周期数据(如新配置未设起效时间)时回退到展示范围的消耗按同一
  // 加权规则分配,保证所有模型都进入排名。
  const rankings = []
  for (const row of keys) {
    ...
    if (row.plan === null || pc === undefined) {
      ...
      for (const m of row.models) {
        ...
        rankings.push({
          key: row.key,
          model: m.model,
          brand: brandLabelOf(providerList, row.provider, m.model),
          usingPrev: false,
          collecting: false,
          tokens,
          amount,
    ...
    // 0.3.7:加权金额 = 订阅费 × (该模型金额 ÷ 周期总金额);各模型金额缺失
    // (全 0 价)时回退旧的 tokens 占比摊薄,保证仍能给出排名
    let srcCost = 0
    for (const m of src.values()) srcCost += m.cost || 0
    for (const m of src.values()) {
      const tokens = m.tokens
      if (!(tokens > 0)) continue
      const amount = srcCost > 0
        ? fee * (m.cost || 0) / srcCost
        : (totalTokens > 0 ? fee * tokens / totalTokens : 0)
      rankings.push({
        key: row.key,
        model: m.model,
        brand: brandLabelOf(providerList, row.provider, m.model),
