 ...
 *
 * 峰谷(0.3.5):主体(模型集体 / 模型×Provider)可配置峰谷规则列表
 * models[model].peak / .providers[pid].peak = { enabled, rules: [{ windows,
 * weekdaysOnly, multiplier }] },条件格式式——自上而下,首个命中时刻的规则按
 * multiplier 缩放基础价,无命中按基础价;主体规则优先于价内嵌 peak 子档
 * (DeepSeek 官方目录档)。无主体规则时沿用内嵌档 + 全局峰谷窗口。
 *
...
/** 归一化主体峰谷规则(0.3.5,条件格式式:自上而下,首个命中规则生效)。 */
...
/**
 * 取某模型的单价(0.3.5):Provider ID 专属价(取消「使用集体价格」时)→
 * 已激活集体价 → 官方目录价;字段级继承 → 全局默认价。key 参数 = Provider ID
 * (账目归一身份,0.3.5 起参与专属价解析;旧调用传 env 标签时按集体价/官方价)。
...
export function priceFor(pricing, key, model) {
  ...
  // 0.3.5:主体峰谷规则随价传递(Provider 主体配置优先于模型集体配置)
  ...
  if (subj !== null && subj.enabled === true && Array.isArray(subj.rules) && subj.rules.length > 0) out.peakRules = subj.rules
  return out
}
 ...
 * 命中调用时刻的规则按 multiplier 缩放基础价,无命中按基础价;旧账目一律基础价。
...
export function tierForEntry(price, atMs, peakCfg, legacy) {
  ...
  if (Array.isArray(price.peakRules) && price.peakRules.length > 0) {
    ...
    const hit = price.peakRules.find((r) => r !== null && typeof r === 'object' && isPeakAt(atMs, r.windows, r.weekdaysOnly))
