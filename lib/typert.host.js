...
import { z } from 'zod'
  ...
  // 0.3.5:主体峰谷规则(条件格式式:自上而下,首个命中规则生效)
  // 0.3.7:tz = 主体时区(UTC 偏移小时,默认 8);规则含生效日(dayScope/days,
  // 兼容旧 weekdaysOnly)与峰时价格方式(mode = multiplier 倍率 / custom 自定义价)
  const subjectPeakSchema = z.object({
    enabled: z.boolean(),
    tz: num.optional(),
    rules: z.array(z.object({
      windows: z.array(z.object({ start: num, end: num })),
      weekdaysOnly: z.boolean().optional(),
      dayScope: z.enum(['weekday', 'everyday', 'custom']).optional(),
      days: z.array(num).optional(),
      mode: z.enum(['multiplier', 'custom']).optional(),
      multiplier: num,
      peakPrice: priceSchema.optional(),
    })),
  })

  // 0.3.5:Provider ID 子价(useCollective = 是否使用集体价格,默认 true)
  // 0.3.7:multiplier = 该 Provider ID 的价格倍率(默认 1)
  const providerPriceSchema = z.object({
    useCollective: z.boolean(),
    price: priceSchema,
    multiplier: num.optional(),
    peak: subjectPeakSchema.optional(),
  })

  // 0.3.1:模型价格跨 Key 合并;enabled=false(未激活)的模型按官方价计费,不入库
  // 0.3.5:providers = Provider ID 子价;peak = 主体(集体)峰谷规则
  // 0.3.7:mode = 自定义方式(custom 逐字段价 / multiplier 倍率),multiplier = 倍率
  const modelPricingSchema = z.object({
    enabled: z.boolean(),
    price: priceSchema,
    mode: z.enum(['custom', 'multiplier']).optional(),
    multiplier: num.optional(),
    providers: z.record(z.string(), providerPriceSchema).optional(),
    peak: subjectPeakSchema.optional(),
  })
  ...
  // keyEnv = 环境变量名(辅助展示),models = 该 Provider 所含模型
  // (0.3.7:{ id, name },name = 全局模型设置里的模型显示名,未配置为空)。
  ...
  // 内置官方价格目录(双币种 + 峰谷子档);0.3.7:name = 显示名(显示层优先于模型 ID)
